// =============================================================
//  SIGNAL SCANNER 24 JAM — v18.9
//  Sniper 8/8 & Simons 6/6 VALID → alert/paper/auto (spt sebelumnya)
//  UPDATE v18.9 (murni PENCATAT — logika sinyal TIDAK berubah):
//   • Bitmap kondisi bernama di setiap sinyal (ctx.conds + ctx.failed)
//   • RESEARCH RECORDER: near-miss Sniper 7/8 & Simons 5/6 dicatat
//     sebagai posisi paper RISET ke rec.researchQueue (ctx.research=1)
//   • Flag ctx.inWindow (0/1) utk analisis jam/hari
//   • Cap riset: 12 posisi terbuka per gaya (Scalping/Intraday/Swing)
//   • Riset TANPA notifikasi Telegram (anti-spam); batch 1x tulis bin
...
//    SCAN_TFS (default "15m,1h"), APP_URL (link di pesan)
...
const RESEARCH_CAP_PER_STYLE = 12; // --- UPDATE v18.9: maks posisi RISET terbuka per gaya ---
...
// --- UPDATE v18.9: parameter `extra` digabung ke ctx (research/inWindow/conds/failed/candleT/rr) ---
function buildPaperItem(sym, side, tf, a, mode, sm, settings, grade, extra){
...
// --- UPDATE v18.9: RESEARCH RECORDER — tulis batch posisi riset ke rec.researchQueue ---
// TANPA Telegram. Dedup per pair+tf+mode+candle. Cap 12 terbuka per gaya. Prune 48 jam.
async function researchRecord(items){
  if(!items.length) return {recorded:[],skipped:0};
  const id=process.env.JSONBIN_ID, key=process.env.JSONBIN_KEY;
  const j=await (await fetch("https://api.jsonbin.io/v3/b/"+id+"/latest",{headers:{"X-Master-Key":key}})).json();
  const rec=j.record||{};
  const now=Date.now();
  rec.researchQueue=(rec.researchQueue||[]).filter(x=>x&&x.openedAt&&now-x.openedAt<48*3600*1000);
  const recorded=[]; let skipped=0;
  for(const item of items){
    const c=item.ctx||{};
    const dupCandle=rec.researchQueue.find(x=>{const xc=x.ctx||{};
      return x.pair===item.pair&&xc.tf===c.tf&&xc.mode===c.mode&&xc.candleT===c.candleT;});
    const dupOpen=rec.researchQueue.find(x=>{const xc=x.ctx||{};
      return x.pair===item.pair&&xc.tf===c.tf&&xc.mode===c.mode&&x.status!=="closed";});
    const openStyle=rec.researchQueue.filter(x=>x.style===item.style&&x.status!=="closed").length;
    if(dupCandle||dupOpen||openStyle>=RESEARCH_CAP_PER_STYLE){ skipped++; continue; }
    rec.researchQueue.push(item);
    recorded.push(c.mode+" "+item.pair+" "+item.dir+" "+(c.tf||"")+(c.nearMiss?" (7/8|5/6)":"")+(c.inWindow?"":" [luar-window]"));
  }
  if(recorded.length){ rec._t=now; await putBin(rec); }
...
// --- UPDATE v18.9: bitmap kondisi bernama; syarat VALID = 8/8 TIDAK BERUBAH ---
function sniperCheck(a, klClosed, htf, btc, tf, settings) {
  if (a.cls === "netral") return { valid: false, neutral: true };
  ...
  const conds = {
    htf:    cfg.useHtf ? (long ? htf.bull : !htf.bull) : true,
    skor:   Math.abs(a.score) >= cfg.minScore,
    smc:    gr.k >= cfg.minSmc,
    adx:    adxV >= cfg.adxMin,
    rsiHtf: long ? htf.rsi < cfg.rsiHi : htf.rsi > cfg.rsiLo,
    macd:   cfg.useMacd ? (long ? a.v.macd > 0 : a.v.macd < 0) : true,
    btc:    cfg.useBtc ? (btc == null ? true : (btc.neutral || (long ? btc.bull : !btc.bull))) : true,
    window: scalpTF ? (cfg.useWindow ? sess.trade : true) : true
  };
  const failed = Object.keys(conds).filter(k => !conds[k]);
  return { valid: failed.length === 0, passed: 8 - failed.length, total: 8, conds, failed,
    side: long ? "LONG" : "SHORT", adx: adxV, grade: gr.g, sess: sess.label, inWindow: sess.trade ? 1 : 0 };
}
// --- UPDATE v18.9: bitmap kondisi bernama; syarat VALID = 6/6 TIDAK BERUBAH ---
function simonsCheck(a, tf, settings) {
  ...
  const sess = sessionStatus(settings);
  const conds = {
    adxLow:  (a.adx || 0) < cfg.adxMax,
    z:       Math.abs(z) >= cfg.zMin,
    rsi:     dir === "SHORT" ? a.rsi >= cfg.rsiS : a.rsi <= cfg.rsiL,
    trigger: trigger,
    noVolAgainst: !momAgainst,
    window:  scalpTF ? sess.trade : true
  };
  const failed = Object.keys(conds).filter(k => !conds[k]);
  return { valid: failed.length === 0, passed: 6 - failed.length, total: 6, conds, failed,
    side: dir, z, sess: sess.label, inWindow: sess.trade ? 1 : 0 };
}
// --- UPDATE v18.9: level struktural Simons (SL/TP/mean/RR) — util bersama valid & riset ---
function simonsLevels(kl, a, side){
  ...
  const sm={ sl: side==="SHORT"? hi10+buf : lo10-buf, tp2: mean, mean };
  sm.tp1 = a.last + (mean-a.last)*0.5;
  sm.rr = Math.abs(sm.tp2-a.last)/Math.abs(a.last-sm.sl);
  return sm;
}
...
module.exports = async (req, res) => {
  ...
  try {
    ...
    const tfs = ((req.query && req.query.tf) || process.env.SCAN_TFS || process.env.SCAN_TF || "15m,1h").split(",").map(x=>x.trim()).filter(Boolean).slice(0,4);
    ...
    // --- UPDATE v18.9: perekam riset aktif di mode paper ATAU auto (butuh jsonbin) ---
    const researchOn = settings.autoPilot !== "off" && !!process.env.JSONBIN_ID && !!process.env.JSONBIN_KEY;
    const researchItems = []; // dikumpulkan dulu, ditulis batch 1x di akhir (hemat kuota jsonbin)
    const paperLog=[];
    ...
    for (const sym of pairs) {
      try {
          ...
          if(paperOn){
            try{
              const it=buildPaperItem(sym, snp.side, tf, a, "SNIPER", null, settings, snp.grade,
                {research:0, nearMiss:0, inWindow:snp.inWindow, conds:snp.conds, failed:[], candleT}); // --- UPDATE v18.9 ---
        ...
        // --- UPDATE v18.9: RESEARCH RECORDER — near-miss SNIPER 7/8 (tanpa Telegram) ---
        else if (researchOn && !snp.neutral && snp.passed === 7 && shouldSend("rsnp-" + sym + "-" + tf, candleT)) {
          try{
            const it=buildPaperItem(sym, snp.side, tf, a, "SNIPER", null, settings, snp.grade,
              {research:1, nearMiss:1, inWindow:snp.inWindow, conds:snp.conds, failed:snp.failed, candleT});
            it.setup="Riset SNIPER";
            researchItems.push(it);
          }catch(e){ paperLog.push("RISET GAGAL "+sym+": "+e.message.slice(0,40)); }
        }
        if (smv.valid && shouldSend("smv-" + sym + "-" + tf, candleT)) {
          hits.push(sym + " SIMONS " + tf);
          if(paperOn){
            try{
              const smP=simonsLevels(kl, a, smv.side); // --- UPDATE v18.9: pakai util bersama ---
              if(smP.rr>=0.8){
                const it=buildPaperItem(sym, smv.side, tf, a, "SIMONS", smP, settings, "MR",
                  {research:0, nearMiss:0, inWindow:smv.inWindow, conds:smv.conds, failed:[], candleT, rr:+smP.rr.toFixed(2)}); // --- UPDATE v18.9 ---
                const okP=await paperRecord(it, chat, appUrl);
          ...
          if(autoOn && guardBlocks && guardBlocks.length===0 && !gs.openSyms.includes(sym)){
            try{
              ...
              const sm=simonsLevels(kl, a, smv.side); // --- UPDATE v18.9: pakai util bersama ---
              if(sm.rr<0.8) throw new Error("R:R ke mean < 0.8");
        ...
        // --- UPDATE v18.9: RESEARCH RECORDER — near-miss SIMONS 5/6 (tanpa Telegram) ---
        else if (researchOn && smv.passed === 5 && shouldSend("rsmv-" + sym + "-" + tf, candleT)) {
          try{
            const smP=simonsLevels(kl, a, smv.side);
            if(smP.rr>=0.8){
              const it=buildPaperItem(sym, smv.side, tf, a, "SIMONS", smP, settings, "MR",
                {research:1, nearMiss:1, inWindow:smv.inWindow, conds:smv.conds, failed:smv.failed, candleT, rr:+smP.rr.toFixed(2)});
              it.setup="Riset SIMONS";
              researchItems.push(it);
            }
          }catch(e){ paperLog.push("RISET GAGAL "+sym+": "+e.message.slice(0,40)); }
        }
    ...
    // --- UPDATE v18.9: tulis batch posisi riset (1x read-modify-write) ---
    let research;
    if(researchOn){
      try{ research = await researchRecord(researchItems); }
      catch(e){ research = { error: e.message.slice(0,60) }; }
    }
    return res.json({ ok: true, tfs, pairs: pairs.length, hits, scanned,
      paper: paperOn ? {active:true, recorded:paperLog} : undefined,
      research: researchOn ? research : undefined,
